...
class ModelToolExportImport extends Model {
	...
	protected function validateProductIdColumns( &$reader ) {
		...
		$product_ids = array();
		$k = $data->getHighestRow();
		$msg = str_replace( '%1', $k, 'Export/Import: HighestRow \'%1\'!');
		$this->log->write( $msg );
		for ($i=1; $i<$k; $i+=1) {
			$product_id = trim($this->getCell($data,$i,1));
			if ($product_id=="") {
				if (!$has_missing_product_ids) {
					$msg = str_replace( '%2', $i, str_replace( '%1', 'Products', $this->language->get( 'error_missing_product_id' ) ) );
					$this->log->write( $msg );
		...
		foreach ($worksheets as $worksheet) {
			...
			$k = $data->getHighestRow();
			$msg = str_replace( '%1', $k, 'Export/Import: 2 HighestRow \'%1\'!');
                	$this->log->write( $msg );
			for ($i=1; $i<$k; $i+=1) {
				$product_id = trim($this->getCell($data,$i,1));
				if ($product_id=="") {
					if (!$has_missing_product_ids) {
						$msg = str_replace( '%2', $i, str_replace( '%1', $worksheet, $this->language->get( 'error_missing_product_id' ) ) );
						$this->log->write( $msg );
						$has_missing_product_ids = true;
					}
					$ok = false;
...
//		}
	...
	public function getNotifications() {
		...
		$result = $this->curl_get_contents("http://www.mhccorp.com/index.php?route=information/message&type=tool_export_import_2_28&language_code=$language_code");
