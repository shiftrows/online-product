...
class ModelCatalogNews extends Model {
	public function addNews($data, $ifcopy = 0) {
		...
		foreach (@$data['news_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "sb_news_description SET news_id = '" . (int)$news_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', description = '" . $this->db->escape($value['description']) . "', meta_desc = '" . $this->db->escape($value['meta_desc']) . "', meta_key = '" . $this->db->escape($value['meta_key']) . "', ctitle = '" . $this->db->escape($value['ctitle']) . "', ntags = '" . $this->db->escape($value['ntags']) . "', cfield1 = '" . $this->db->escape($value['cfield1']) . "', cfield2 = '" . $this->db->escape($value['cfield2']) . "', cfield3 = '" . $this->db->escape($value['cfield3']) . "', cfield4 = '" . $this->db->escape($value['cfield4']) . "'");
		}
	...
	public function getNewsDA() {
		$query = $this->db->query("SELECT date_added, store_id, n.news_id FROM " . DB_PREFIX . "sb_news n LEFT JOIN " . DB_PREFIX . "sb_news_to_store n2s ON (n.news_id = n2s.news_id) LEFT JOIN " . DB_PREFIX . "sb_news_description nd ON (n.news_id = nd.news_id) WHERE nd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND n.status = '1' ORDER BY n.date_added");
	
		return $query->rows;
	}
	
	public function getNewsDU() {
		$query = $this->db->query("SELECT date_updated, store_id, n.news_id FROM " . DB_PREFIX . "sb_news n LEFT JOIN " . DB_PREFIX . "sb_news_to_store n2s ON (n.news_id = n2s.news_id) LEFT JOIN " . DB_PREFIX . "sb_news_description nd ON (n.news_id = nd.news_id) WHERE nd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND n.status = '1' ORDER BY n.date_updated");
	
		return $query->rows;
	}
	
	public function getNewsDP() {
		$query = $this->db->query("SELECT date_pub, store_id, n.news_id FROM " . DB_PREFIX . "sb_news n LEFT JOIN " . DB_PREFIX . "sb_news_to_store n2s ON (n.news_id = n2s.news_id) LEFT JOIN " . DB_PREFIX . "sb_news_description nd ON (n.news_id = nd.news_id) WHERE nd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND n.status = '1' ORDER BY n.date_pub");
